...
public interface FacturaRepository extends JpaRepository<FacturaEntity, Long>{
    ...
    @Query(value= "SELECT ROUND(SUM(factura.monto) * 0.19,0) FROM silvahnos.factura WHERE factura.borrado = 0 AND year(factura.fecha_creacion) = :anio AND month(factura.fecha_creacion) = :mes", nativeQuery = true)
